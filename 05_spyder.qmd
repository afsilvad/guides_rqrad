...
Il est tentant d'installer Spyder directement dans l'environnement `base`, ou dans chaque environnement de projet créé. Ce n'est cependant pas la méthode recommandée par l'équipe de Spyder elle-même.

::: {.callout-important}
N'installez jamais Spyder dans l'environnement `base`, et éviter de le réinstaller dans chaque environnement de projet. Installer Spyder à répétition consomme beaucoup d'espace disque et de temps, et peut provoquer des conflits entre paquets d'un environnement à l'autre.
:::
...
Cette approche permet de toujours lancer Spyder de la même façon, peu importe le projet sur lequel vous travaillez, tout en gardant vos environnements de projet légers et isolés.
...
Cette commande crée un nouvel environnement nommé `spyder-env`, en installant Spyder depuis le canal `conda-forge`, généralement plus à jour.
...
Ce n'est toutefois pas obligatoire : ces paquets peuvent tout aussi bien vivre uniquement dans vos environnements de projet.
:::
...
Cette étape n'est pas obligatoire pour démarrer, mais elle évite des conflits de paquets lors de mises à jour futures.

## Étape 3 : ouvrir Spyder

Lorsque vous installez Spyder par conda, un raccourci est créé **automatiquement** dans le menu Démarrer de Windows (ou le lanceur d'applications sur macOS/Linux). Vous n'avez donc rien à construire vous-même pour ouvrir Spyder normalement.
...
### Créer un raccourci personnalisé (optionnel, avancé)

Il est possible de créer manuellement un raccourci Windows qui active un environnement précis avant de lancer une commande, par exemple si vous voulez démarrer directement dans un environnement de projet particulier plutôt que dans `spyder-env`.

1. Faites un clic droit sur le bureau, puis choisissez **Nouveau > Raccourci**.
2. Dans le champ d'emplacement, entrez une ligne semblable à celle-ci, en adaptant le chemin à votre installation :

```
%windir%\System32\cmd.exe /K C:\Users\votre_nom\miniconda3\Scripts\activate.bat spyder-env
```

3. Cliquez sur **Suivant**, donnez un nom clair au raccourci, puis **Terminer**.

::: {.callout-note}
Cette méthode manuelle n'est généralement pas nécessaire, puisque le raccourci automatique créé à l'installation suffit pour la grande majorité des utilisations. Réservez-la à des cas particuliers, par exemple si vous gérez plusieurs environnements Spyder distincts sur le même ordinateur.
:::
...
::: {.callout-important}
N'installez jamais Spyder lui-même (le paquet complet) dans un environnement de projet. Seul `spyder-kernels` est nécessaire pour que Spyder puisse s'y connecter.
:::
...
2. Cliquez sur **Change default environment in Preferences...** (ou ouvrez directement **Tools > Preferences > Python interpreter**).
3. Dans la section **Python interpreter**, sélectionnez l'option **Select interpreter**, puis choisissez votre environnement dans la liste déroulante si celui-ci y apparaît automatiquement.
4. S'il n'apparaît pas dans la liste, utilisez le bouton **Select file**, ou collez directement le chemin copié à l'étape précédente dans le champ de texte prévu.
...
Spyder lui-même n'a besoin d'être installé qu'une seule fois, dans `spyder-env`, peu importe le nombre de projets créés par la suite.

::: {.callout-tip}
Une fois plusieurs environnements ajoutés, ils restent généralement visibles dans la liste déroulante des interpréteurs, ce qui permet de basculer d'un projet à l'autre sans avoir à retrouver le chemin complet chaque fois.
:::
